...
var Multiplayer = {
  ...
  ping: function () {
    ...
    for (var name in Multiplayer.props) {
      ps +=
        (ps == '' ? '' : '&') + name + '=' + escape(Multiplayer.props[name]);
    ...
    for (var i = 0; i < Multiplayer.events.length; i++)
      ps +=
        (ps == '' ? '' : '&') +
        Multiplayer.events[i][0] +
        '=' +
        escape(Multiplayer.events[i][1]);
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4 && window && window.Multiplayer)
        Multiplayer.handleResponse(xhr.responseText);
    };
    xhr.send(ps);
    Multiplayer.props = {};
    Multiplayer.events = [];
