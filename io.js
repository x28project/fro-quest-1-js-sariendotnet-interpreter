...
var IO = {
  ...
  onKeyDown: function (evt) {
    ...
    if (Agent.OP) IO.cancelNextKeyClick = true;

    var evt = Agent.IE ? event : evt;
    var key = evt.keyCode;

    // always cancel the "/" quick search
    if (key == 191 && !evt.shiftKey) Agent.cancelEvent(evt);
    ...
    if (IO.actionsVisible()) {
      if (key == 37) IO.executeAction(IO.backAction);
      if (key == 13 || key == 32 || key == 39) IO.chooseAction();
      if (key == 27) IO.hideActions();
    ...
    if (evt.altKey && evt.ctrlKey && key == 65) IO.addAllAvatars();
    ...
    switch (evt.keyCode) {
      case 27:
        if (IO.input == '') return Menu.showHelp();
        IO.input = '';
        IO.showCommandLine();
        break;
      case 13:
        if (Utils.Trim(IO.input).length == 0) return;
        var input = IO.input;
        IO.input = '';
        IO.showCommandLine();
    ...
    if (new_ego_dir) {
      ...
      if (new_ego_dir == current_ego_dir) new_ego_dir = stopped;

      if (Test.recording) Test.recordDirection(new_ego_dir);
  ...
  onKeyPress: function (evt) {
    ...
    IO.cancelNextKeyPress = false;
    if (IO.actionsVisible() || Text.messageShown) return;
  ...
  showCommandLine: function () {
    if (IO.input.length < IO.maxInputLength) {
      ...
      var cmdPrompt = '>';
      if (strings[0] && strings[0].length == 1) cmdPrompt = strings[0];
  ...
  onClick: function (evt, dbl) {
    if (Sound.playClick && Sound.playClickDone == false) {
      Sound.loadAll();
    ...
    if (AGI.paused) {
      //if (Text.messageShown)
      //  Text.hideMessage();
      if (IO.actionsVisible()) IO.hideActions();
      cmd_reset(flag_input_received);
      cmd_assignn(var_unknown_word_no, 0);
      cmd_set(flag_input_parsed);
      if (evt) {
    ...
    var x =
      Math.min(Math.max(evt.clientX, canvas.left), canvas.right) - canvas.left;
    var y =
      Math.min(Math.max(evt.clientY, canvas.top), canvas.bottom) - canvas.top;
    x = Math.round(x / 4);
    y = Math.round(y / 2);
    var ego = getEgo();
  ...
  showActions: function () {
    ...
    if (x + IO.actions.offsetWidth > screenWidth) x -= IO.actions.offsetWidth;
  ...
  parseCommandLine: function (cmd) {
    ...
    while (cmd.length > 0) {
      ...
      cmd = Utils.Trim(cmd.toLowerCase().replace(/\s+/g, ' ')).replace(
        /[^\w ]/g,
        ''
      );

      // ignore specific messages that are common in multiplayer
      if (Utils.matches(cmd, ['^hello\\b', '^hi\\b', '^hey\\b'])) return;
      ...
      for (var j = parts.length; j >= 0; j--) {
        ...
        for (var testToken in TOKENS) {
          if (
            Utils.ArrayHasItem(TOKENS[testToken], token) &&
            token.length > longestToken.length
          ) {
            longestToken = token;
            tokenFound = true;
          }
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      for (var testToken in TOKENS) {
        if (
          /*testToken != "0" &&*/ Utils.ArrayHasItem(TOKENS[testToken], token)
        ) {
          if (testToken != '0') {
      ...
      if (tokenFound === false) {
        ...
        cmd_assignn(
          var_unknown_word_no,
          IO.lastInput.split(' ').indexOf(token) + 1
        );
      }
    }
  ...
  parse: function (input, fromCommandLine) {
    ...
    if (!fromCommandLine) input = Hacks.parse(AGI.game_id, input);
    if (input == a_separator) return;
    if (input.indexOf('#') == 0) return Sarien.checkForHashChange(input);
    if (input.indexOf('$') == 0) return AGI.setAvatar(input.substr(1) * 1);
    if (input.match(/^F\d+$/)) return IO.chooseFKey(input.substr(1) * 1);
    ...
    if (!input || input == '') {
      ...
      if (Test.recording) Test.recordAction(input);
  ...
  hasSaid: function (args) {
    ...
    var saidText = IO.said
      .join(' ')
      .toLowerCase()
      .replace(/\srol$/g, '')
      .split(' ');
    var checkText = argArr
      .join(' ')
      .toLowerCase()
      .replace(/\srol$/g, '')
      .split(' ');

    if (rolIndex > -1 && rolIndex < IO.said.length) {
      saidText = IO.said.slice(0, rolIndex).join(' ').toLowerCase().split(' ');
      checkText = argArr.slice(0, rolIndex).join(' ').toLowerCase().split(' ');
    }

    if (saidText.length != checkText.length) return false;
  ...
  parseRoomCommands: function (commands) {
    ...
    for (var i = 0; i < commands.length; i++) {
      ...
      if (!IO.commands[verb]) IO.commands[verb] = {};
      if (commands.length > 1) {
        var subject = '';
        for (var j = 1; j < command.length; j++) {
          subject += Utils.Trim(command[j]);
          if (j < command.length - 1) subject += ' ';
        }
  ...
  executeAction: function (verb) {
    ...
    if (!back) IO.prevSelectedIndex = IO.actions.selectedIndex;
    IO.verb = verb;
    ...
    if (IO.usingKeyboard) {
      ...
      if (back) desiredSelectedIndex = IO.prevSelectedIndex;
      // and set it
  ...
  getCommands: function (logicNr, asObj) {
    ...
    s.replace(reg1, function (a, b) {
      ...
      var saids = a.replace(reg2, function (c, d) {
        ...
        if (
          said.join().length > bestSaid.join().length &&
          said.join().indexOf('anyword') == -1
        )
          bestSaid = said;
      });
  ...
  getPrettyVerb: function (verb) {
    ...
    while (verb.length > 0 && verb.indexOf(' ') != -1) {
      ...
      if (longestVerb === null) {
        ...
        if (longestVerb == '') longestVerb = token = parts[0];
    ...
    if (verb.length > 0) {
      if (IO.prettyVerbs[verb]) verbs.push(IO.prettyVerbs[verb]);
  ...
  addPrettyVerbs: function (prettyVerbs) {
    for (var p in prettyVerbs) IO.prettyVerbs[p] = prettyVerbs[p];
  },
  // gives you instant access to all avatars
  addAllAvatars: function () {
    for (var id = 0; id < 256; id++) if (avatarNames[id]) IO.avatars[id] = true;
    ...
    if (IO.commandLineIsVisible) {
      ...
      for (var inventoryCommand in IO.inventoryCommands) {
        if (Array.isArray(IO.inventoryCommands[inventoryCommand])) {
          ...
          for (var inventoryCommandArray in IO.inventoryCommands[
            inventoryCommand
          ]) {
            if (
              cmd_has(
                IO.inventoryCommands[inventoryCommand][inventoryCommandArray]
              )
            ) {
  ...
  showSubActions: function (verb) {
    ...
    IO.addOption(
      '&lt;',
      IO.commonCommandsActive ? a_global_verbs : a_local_verbs
    );
    var arr = [];
    for (var subject in IO.commands[verb]) arr.push(IO.getPrettyVerb(subject));
    arr.sort();
    for (var i = 0; i < arr.length; i++)
