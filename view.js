...
function getObject(n) {
  ...
  if (n > AGI.highestObjIndex) AGI.highestObjIndex = n;
  return obj;
}
...
View.prototype = {
  ...
  load: function (id) {
    ...
    this.id = id;
    var imagePath =
      Sarien.path + '/view' + Utils.PadLeft(this.id, '0', 3) + '.png';
  ...
  setDestination: function (x, y) {
    ...
    if (this.isEgo()) vars[var_ego_dir] = this.direction;
  },
  ...
  hide: function () {
    if (this.isVisible()) this.rootElement.style.visibility = 'hidden';
  },
  ...
  checkMotion: function () {
    ...
    if (AGI.block && this.observe_blocks && this.direction) this.changePos();
  },

  motionWander: function () {
    if (!this.direction || this.DIDNT_MOVE) this.direction = Utils.Random(1, 8);
    if (this.isEgo()) vars[var_ego_dir] = this.direction;
  },
  ...
  motionMoveObj: function () {
    ...
    if (this.direction == 0) this.inDestination();
  },
  ...
  changePos: function () {
    ...
    if (this.checkBlock(x, y) != b) {
      this.direction = 0;
      if (this.isEgo()) vars[var_ego_dir] = 0;
    }
  },
  ...
  inDestination: function () {
    ...
    if (this.motion_type != mt_wander) this.motion_type = mt_normal_motion;

    if (this.isEgo()) AGI.control = c_player_control;
  },

  checkBlock: function (x, y) {
    if (x <= AGI.block.x1 || x >= AGI.block.x2) return false;

    if (y <= AGI.block.y1 || y >= AGI.block.y2) return false;
  ...
  updateViewTableEntry: function () {
    ...
    if (loop != 4 && loop != this.loop) {
      if (this.step_time_count == 1) this.loop = loop;
    }

    if (!this.CYCLING) return;

    if (this.cycle_time_count == 0) return;
  ...
  updateCel: function () {
    ...
    switch (this.cycle_type) {
      case ct_normal_cycle:
        if (++cel > lastCel) cel = 0;
        break;
      case ct_end_of_loop:
        if (cel < lastCel) {
          if (++cel != lastCel) break;
        }
        /*if (this.flag_to_set > -1) {
          cmd_set(this.flag_to_set);
          this.flag_to_set = -1;
  ...
  updatePosition: function () {
    ...
    if (this.step_time_count != 0) {
      if (--this.step_time_count != 0) return;
    }
    ...
    if (this.x == old_x && this.y == old_y) {
      if (this.didnt_move_count) this.direction = 0;
      else this.didnt_move_count++;
    } else this.didnt_move_count = 0;
    ...
    if (border != 0) {
      ...
      if (this.motion_type == mt_move_obj && old_x !== 0 && old_y !== 0) {
        this.inDestination();
      }
    }
  ...
  xyok: function () {
    return (
      !this.checkCollision() && this.checkPriority() && this.checkPosition()
    );
  },
  ...
  checkCollision: function () {
    if (!this.observe_objects) return false;
    ...
    if (this.priority != 15) {
      ...
      for (i = 0, w = this.width(); i < w; i++) {
        ...
        if (pri == 3) {
          ...
          for (
            var j = 0, priWaterLength = priWater.length;
            j < priWaterLength;
            j++
          ) {
            if (priWater[j] === 0) {
        ...
        /* conditional blue */
        if (pri == 1) {
          if (!this.observe_blocks) continue;
      ...
      if (pass) {
        if (!water && this.ON_WATER) pass = 0;
        if (water && this.ON_LAND) pass = 0;
      }
    }
