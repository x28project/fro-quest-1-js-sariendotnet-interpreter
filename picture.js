...
Picture.prototype = {
  ...
  addStaticObject: function (obj) {
    ...
    if (obj.margin < 4) {
      ...
      var checkY = obj.y - 1;
      while (checkY > 0) {
        if (AGI.priorityTable[checkY] != prio) break;
        prioBandTop = checkY--;
      }
  ...
  isInObjectMargin: function (x, y, obj) {
    if (x < obj.x || x > obj.x + obj.width()) return false;

    if (y < obj.priorityBandTop || y > obj.y) return false;
