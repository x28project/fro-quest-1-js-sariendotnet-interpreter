...
var State = {
  ...
  getState: function () {
    ...
    // ego info
    state +=
      ego.x + ',' + ego.y + ',' + ego.id + ',' + ego.loop + ',' + ego.cel + '|';

    // vars
    for (var i = 0; i < 256; i++) {
      var value = vars[i];
      if (value > 0) state += i + ',' + value + ',';
    }
    state = state.replace(/,$/, '');
  ...
  save: function () {
    ...
    document.title =
      'Fro Quest: "' +
      Sarien.getRoomName(AGI.current_room) +
      '", Score: ' +
      Menu.score;
  ...
  load: function () {
    ...
    for (var i = 0; i < varInfo.length; i += 2)
      varObj[varInfo[i] * 1] = varInfo[i + 1] * 1;
    for (var i = 0; i < 256; i++) vars[i] = varObj[i] ? varObj[i] : 0;

    // flags
    var flagInfo = parts[3].split(',');
    var flagObj = {};
    for (var i = 0; i < flagInfo.length; i++) flagObj[flagInfo[i] * 1] = true;
    for (var i = 0; i < 256; i++) flags[i] = flagObj[i] ? true : false;

    // inventory
    var invInfo = parts[4].split(',');
    window.items = {};
    for (var i = 0; i < invInfo.length; i++) items[invInfo[i] * 1] = true;
